...
module.exports = async (req,res) => {
        try {
        const body = req.body
        const tableau = [{name : "name",value : body.name},{name : "about_all_road",value : body.description},{name : "presentation_image",value : req.file ? req.file.filename : undefined},{name : "prix", value : body.price},{name : "period", value : `${body.period_B} ${body.period_E}`},{name : "dificulter",value : body.difficulty},{name : "distance", value : body.distance},{name : "sejours_delay", value : `${body.sejours_delais_B} ${body.sejours_delais_E}`}, {name : "confort", value : body.confort},{name : "parent_id",value : body.parent_id}]
        const data = async () => {
            return {
